 ...
 */
 ...
 static int extract_struct_offsets_from_btf(const char *kimg, int kimg_len, struct_offsets_t *offsets)
 {
 ...
 
     {
        ...
        offsets->task_struct_active_mm_offset = task_struct_members.offset;
    }
...
/**
 * 验证输出文件中的 struct_offsets 是否写入成功
 * @param file_path 输出文件路径
 * @param preset_offset preset 在文件中的偏移量
 * @param expected_offsets 期望的 struct_offsets 数据（用于比较）
 * @param kernel_is_be 内核字节序
 * @return 0 成功，-1 失败
 */
static int verify_struct_offsets_in_file(const char *file_path, int preset_offset, 
                                         const struct_offsets_t *expected_offsets, bool kernel_is_be)
{
    tools_logi("Verifying struct_offsets in output file: %s\n", file_path);
    
    // 读取输出文件
    char *file_data = NULL;
    int file_len = 0;
    read_file_align(file_path, &file_data, &file_len, 1);
    
    if (file_len < preset_offset + sizeof(preset_t)) {
        tools_loge("File too small: %d < %zu\n", file_len, preset_offset + sizeof(preset_t));
        free(file_data);
        return -1;
    }
    
    // 找到 preset 位置
    preset_t *preset = (preset_t *)(file_data + preset_offset);
    
    // 检查 magic
    char magic[MAGIC_LEN] = KP_MAGIC;
    if (memcmp(preset->header.magic, magic, MAGIC_LEN) != 0) {
        tools_loge("Invalid preset magic in file\n");
        free(file_data);
        return -1;
    }
    
    // 读取文件中的 struct_offsets
    struct_offsets_t file_offsets;
    memcpy(&file_offsets, &preset->setup.struct_offsets, sizeof(struct_offsets_t));
    
    // 注意：文件中的数据已经是正确的字节序（在写入前已经转换过）
    // 所以这里不需要再次转换，直接比较即可
    ...
    #define CHECK_FIELD(field, name) \
        do { \
            total_count++; \
            if (file_offsets.field != expected_offsets->field) { \
                tools_loge("  [FAIL] %s mismatch: file=0x%04x, expected=0x%04x\n", \
                          name, (unsigned)file_offsets.field, (unsigned)expected_offsets->field); \
                failed_count++; \
            } else if (file_offsets.field == -1) { \
                tools_loge("  [FAIL] %s is -1 (not found)\n", name); \
                failed_count++; \
            } else { \
                tools_logi("  [OK]   %s = 0x%04x\n", name, (unsigned)file_offsets.field); \
            } \
        } while (0)
    
    // 检查关键字段
    CHECK_FIELD(task_struct_pid_offset, "task_struct_pid_offset");
    CHECK_FIELD(task_struct_cred_offset, "task_struct_cred_offset");
    CHECK_FIELD(task_struct_real_cred_offset, "task_struct_real_cred_offset");
    CHECK_FIELD(cred_uid_offset, "cred_uid_offset");
    CHECK_FIELD(cred_gid_offset, "cred_gid_offset");
    CHECK_FIELD(cred_cap_effective_offset, "cred_cap_effective_offset");
    CHECK_FIELD(mm_struct_pgd_offset, "mm_struct_pgd_offset");
    
    #undef CHECK_FIELD
    
    // 完整比较所有字段
    if (memcmp(&file_offsets, expected_offsets, sizeof(struct_offsets_t)) != 0) {
        tools_loge("struct_offsets data mismatch in file\n");
        failed_count++;
    }
    ...
    if (failed_count > 0) {
        tools_loge("struct_offsets verification FAILED: %d issue(s) found\n", failed_count);
        return -1;
    }
    
    tools_logi("struct_offsets verification PASSED in output file\n");
    return 0;
}
...
int patch_update_img(const char *kimg_path, const char *kpimg_path, const char *out_path, const char *superkey,
                     bool root_key, const char **additional, extra_config_t *extra_configs, int extra_config_num)
 {
    ...
    // 字节序转换（如果需要）
    if ((is_be() ^ kinfo->is_be)) {
        // struct_offsets中的所有int16_t字段需要字节序转换
        // 只转换实际的int16_t字段，不包括padding
    ...
    // // 验证输出文件中的 struct_offsets 是否写入成功
    // if (verify_struct_offsets_in_file(out_path, align_kimg_len, &setup->struct_offsets, kinfo->is_be) != 0) {
    //     tools_loge_exit("struct_offsets verification failed in output file: %s\n", out_path);
    // }
